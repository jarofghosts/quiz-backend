...
Two middleware functions are provided by default in the `quiz-backend.middleware` namespace.
...
See the `:middleware` key of the `app` definition located in the `quiz-backend.handler` namespace to manage the enabled middleware.
